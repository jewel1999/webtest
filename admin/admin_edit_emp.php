<?php 
    ...
    if(isset($_POST['admin_emp_update'])){
        ...
        $station = $_POST['station'];
                ...
                if(!isset($_SESSION['error'])){
                    $sql = $conn->prepare("UPDATE  staffinfo SET com_sn=:com_sn,com_name=:com_name,cpu=:cpu,
        ...
        <div class="modal-body"> 
            <form action="admin_edit_emp.php" method="post" enctype="multiplepart/form-data">
                <?php 
                    if(isset($_GET['id'])){
                        $id = $_GET['id'];
                        $stmt = $conn->query("SELECT * FROM staffinfo  WHERE id = $id");
                        $stmt->execute();
                        $data = $stmt->fetch();
                    }
                ?>
